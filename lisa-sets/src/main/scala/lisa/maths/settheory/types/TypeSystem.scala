...
object TypeSystem {
  ...
  import TypeLib.{f, x, y, a, any, definition, given}
  ...
  private class FunctionalTypeAssignmentConstant[N <: Arity](val functional: Term ** N |-> Term, val typ: FunctionalClass, formula: ConstantFormula)
      extends ConstantFormula(formula.id)
      with FunctionalTypeAssignment[N]
  private class FunctionalTypeAssignmentPredicate[N <: Arity](val functional: Term ** N |-> Term, val typ: FunctionalClass, formula: AppliedPredicate)
      extends AppliedPredicate(formula.label, formula.args)
      with FunctionalTypeAssignment[N]
  private class FunctionalTypeAssignmentConnector[N <: Arity](val functional: Term ** N |-> Term, val typ: FunctionalClass, formula: AppliedConnector)
      extends AppliedConnector(formula.label, formula.args)
      with FunctionalTypeAssignment[N]
  private class FunctionalTypeAssignmentBinder[N <: Arity](val functional: Term ** N |-> Term, val typ: FunctionalClass, formula: BinderFormula)
      extends BinderFormula(formula.f, formula.bound, formula.body)
      with FunctionalTypeAssignment[N]

  class TypedConstant[A <: Class](id: Identifier, val typ: A, val justif: JUSTIFICATION) extends Constant(id) with LisaObject[TypedConstant[A]] {
  ...
  ) extends ConstantFunctionLabel[N](id, arity)
      with LisaObject[TypedConstantFunctional[N]] {
  ...
  extension (d: definitionWithVars[0]) {
    inline infix def -->[A <: Class](using om: OutputManager, name: sourcecode.FullName, line: sourcecode.Line, file: sourcecode.File)(term: Term, typ: A): TypedConstant[A] =
  ...
  object TypeChecker extends ProofTactic {
    ...
    def typecheck(using lib: SetTheoryLibrary.type, proof: lib.Proof)(context: Seq[Formula], term: Term, typ: Option[Class]): proof.ProofTacticJudgement =
      val typingAssumptions: Map[Term, Seq[Class]] = fullFlat(context)
        .collect { case TypeAssignment(term, typ) =>
          (term, typ)
        }
        .groupBy(_._1)
        .map((t, l) => (t, l.map(_._2)))

      val functionalTypingAssumptions: Map[(? |-> Term), Seq[FunctionalClass]] = context
        .collect { case FunctionalTypeAssignment(func, typ) =>
          (func, typ)
        }
        .groupBy(_._1)
        .map((func, l) => (func, l.map(_._2)))
          ...
          def innerTypecheck(context2: Map[Term, Seq[Class]], term: Term, typ: Option[Class]): Class = {
            ...
            if typ == Some(any) then
              ...
              term match
                ...
                case AppliedFunctional(label, args) =>
                  val (argTypes, argTypesProofs) = args
                    .map(arg =>
                      ...
                      catch case e: TypingException => (any, any.definition of (x := arg)) // if no type could be constructed the normal way, we give it "any"
                    )
                    .unzip
                  ...
                  if labelTypes.isEmpty then throw TypingException("Function " + label + " expected to have type (" + argTypes.mkString(", ") + ") |=> ? but is untyped.")
                  else
                    typ match
                      case None =>
                        labelTypes.find((labelType, step) =>
                          labelType.arity == args.size &&
                            (args zip argTypes)
                              .zip(labelType.in.toSeq)
                              .forall((argAndTypes, inType) => K.isSame((argAndTypes._1 is inType).asFormula.underlying, (argAndTypes._1 is argAndTypes._2).asFormula.underlying) //
                              )
                        ) match
                          case None =>
                            throw TypingException(
                              "Function " + label + " expected to have type (" + argTypes.mkString(", ") + ") |=> ? but is assigned " + labelTypes.mkString(
                                " & "
                              ) + ". Note that terms having multiple different types is only partialy supported."
                            )
                      ...
                      case Some(typValue) =>
                        labelTypes.find((labelType, step) =>
                          ...
                          case None =>
                            throw TypingException(
                              "Function " + label + " expected to have type (" + argTypes.mkString(", ") + ") |=> " + typValue + " but is assigned " + labelTypes.mkString(
                                " & "
                              ) + ". Note that terms having multiple different types is only partialy supported."
                            )
                          case Some(labelType, step) =>
                ...
                case v: Variable =>
                  if possibleTypes.isEmpty then throw TypingException("Variable " + v + " expected to be of type " + typ + " but is untyped.")
                  else throw TypingException("Variable " + v + " expected to be of type " + typ + " but is assigned " + possibleTypes.mkString(" & ") + ".")

                case c: Constant =>
                  if possibleTypes.isEmpty then throw TypingException("Constant " + c + " expected to be of type " + typ + " but is untyped.")
